...
test('toplam kota restart sonrasında sıfırlanmaz', () => {
  ...
  try {
    ...
    assert.equal(ilk.kalan('cihaz-a'), 5);
    for (let i = 0; i < 5; i++) assert.equal(ilk.kullan('cihaz-a'), true);
    assert.equal(ilk.kalan('cihaz-a'), 0);
    assert.equal(ilk.kullan('cihaz-a'), false);

    const yeniden = new ToplamKota({ limit: 5, dosya });
    assert.equal(yeniden.kalan('cihaz-a'), 0);
    assert.equal(yeniden.kalan('cihaz-b'), 5);
